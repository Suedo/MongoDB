    ...
    this.itemInCart = function(userId, itemId, callback) {
         ...
         */
         ...
         this.db.collection('cart').find(
           { "userId": userId },                              // "558098a65133816958968d88"
           { _id:0, items:{ $elemMatch: { _id: itemId } } }   // keep only the matching array element from items[]
         ).limit(1)                                           // should be 1, still, just in case
         .next((err, doc) => {
           if( err ) { callback(null); }                      // pass null if no matching item
